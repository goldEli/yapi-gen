...
import { compile as compileEjs, compileEjsFile, Model } from "../utils/ejs";
...
export const genCodeByYapi = async (
  ...
  try {
    ...
    const p = path.resolve(rootPath, `./yapi-gen/template.ejs`);
    const code = await compileEjsFile(p, model);
    replaceToEditor(code);
  } catch (e: any) {
...
function fixSchema(obj: any, fieldNames: string[] = ["$ref", "$$ref"]) {
