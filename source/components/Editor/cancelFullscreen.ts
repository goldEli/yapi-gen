...
class FullScreenButtonMenu implements IButtonMenu {
  readonly title = currentLanguage === 'zh' ? '取消全屏' : 'Cancel fullscreen'
  ...
  exec(editor: NewIDomEditor) {
    ...
    if (el) {
      ...
      if (editor.changeEditor) {
        editor.changeEditor(false, 'customFullScreen')
      }
    }
  }
}

const cancelfullscreenMenu = {
  key: 'cancelCustomFullScreen',
  factory() {
    return new FullScreenButtonMenu()
  },
}

export default cancelfullscreenMenu
