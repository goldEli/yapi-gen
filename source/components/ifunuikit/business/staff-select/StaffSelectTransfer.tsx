...
import { Avatar, Checkbox, Input, Tree, type TreeDataNode } from 'antd'
...
const StaffSelectTransfer = (props: Props) => {
  ...
  const onSelect = (id: string) => {
    if (props.value?.includes(id)) {
      props.onChange?.(props.value.filter((i: any) => i !== id))
    } else {
      props.onChange?.((props.value || []).concat(id))
    }
  }

  const hasDisableChild = (data: any) => {
    const nodes = [data]
    ...
    const disabledDepartmentIds = staffList
      .filter(item => disabledList?.includes(item.id))
      .map(item => item.departmentId)
    return childrens.some(item => disabledDepartmentIds?.includes(item.id))
  }
  ...
  const appendDepartmentStaffs = (
    ...
    subDepartments.map((i: any) => {
      ...
      const disabled = hasDisableChild(i)
      return {
        title: (
          ...
          (staffList
            ...
            .map(staff => ({
              title: (
                <StaffNode>
                  <Checkbox
                    disabled={disabledList?.includes(staff.id)}
