...
const NewRelatedNeedForProject = (props: any) => {
  ...
  const confirm = async () => {
    ...
    const result = newData.needs.map((i: any) => ({
      ...
      schedule_percent: props?.data?.find((s: any) => s.id === i.id)
        ?.schedule_percent,
      today_task_time: props?.data?.find((s: any) => s.id === i.id)
        ?.today_task_time,
    }))
  ...
  useEffect(() => {
    setChooseList(
      props?.initValue?.map((k: any) => ({
        ...
        schedule_percent: k.schedule_percent,
        today_task_time: k.today_task_time,
      })) ?? [],
    )
...
export default NewRelatedNeedForProject
